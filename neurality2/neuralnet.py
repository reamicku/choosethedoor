...
class NeuralNet():
    def __init__(self, n_inputs: int, n_outputs: int, n_hidden: int, connection_prob: float = 0.5) -> None:
        ...
        self.connections = np.zeros(
            (self.n_total_neurons, self.n_total_neurons))
            ...
            for i in range(len(self.weights)):
                if np.random.rand() < mutation_rate:
                    self.weights[i] = np.random.normal(
                        0, 1, self.weights[i].shape)
        if not skip_biases:
            for i in range(len(self.biases)):
                if np.random.rand() < mutation_rate:
                    self.biases[i] = np.random.normal(
                        0, 1, self.biases[i].shape)
    ...
    def multi_point_crossover(parent1: 'NeuralNet', parent2: 'NeuralNet', num_points: int) -> list['NeuralNet']:
        ...
        crossover_points = np.sort(np.random.choice(
            parent1.n_total_neurons, num_points + 1, replace=False))
    ...
    def get_network_architecture(self):
        ...
        hiddenrange = range(self.n_inputs + self.n_outputs,
                            self.n_total_neurons)
        hiddenNeuronIDs = []
